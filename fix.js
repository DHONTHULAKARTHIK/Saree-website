...
const targetStr = `    // E – Order status timeline
          ...
          \${steps.map((step, i) => {
            ...
            return \\\`
              <div style="display:flex; flex-direction:column; align-items:center; flex:1;">
                <div style="width:26px; height:26px; border-radius:50%; background:\${done ? 'rgba(197,151,58,0.25)' : 'rgba(255,255,255,0.05)'}; border:2px solid \${col}; display:flex; align-items:center; justify-content:center; font-size:0.7rem; color:\${col}; margin-bottom:5px;">\${done ? '✓' : (i+1)}</div>
...
const replacementStr = `    // E – Order status timeline
...
if (code.replace(/\r\n/g, '\n').includes(targetStr.replace(/\r\n/g, '\n'))) {
  code = code.replace(/\r\n/g, '\n').replace(targetStr.replace(/\r\n/g, '\n'), replacementStr);
  fs.writeFileSync('profile.js', code);
  console.log('Replaced successfully');
} else {
  console.log('Target string not found in profile.js');
}
